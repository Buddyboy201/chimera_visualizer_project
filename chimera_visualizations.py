...
def _display_chimera(row, pdb_directory, chimera_path):
    ...
    p = check_call([str(chimera_path), str(path_to_pdb),
                    str(path_to_cmd)], shell=True)
    path_to_cmd.unlink()


def display_chimera(conn, sql_id, pdb_directory, chimera_path):
    stmt_id = "SELECT pdbname, oldresid FROM cliques WHERE id={}".format(
        sql_id)
    row = list(conn.execute(stmt_id))[0]
    t = Thread(target=_display_chimera, args=(
        row, pdb_directory, chimera_path,))
    t.start()
